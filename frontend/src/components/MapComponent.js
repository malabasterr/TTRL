      ...
      const fetchData = async () => {
        try {
          ...
          connectionData.forEach((connection) => {
            const sourceCity = cityData.find(
              (city) => city.id === connection.start_city_id,
            );
            const destinationCity = cityData.find(
              (city) => city.id === connection.end_city_id,
            );
            ...
            if (sourceCity && destinationCity) {
              ...
              if (connection.team_claims.length > 0) {
                ...
                if (claimedTeamId === "79cd421b-81d4-4b00-8b59-da9e7560dc4b") {
                  ...
                  claimedTeamId === "1446e8a4-350c-4aa1-a997-c05fb87ef102"
                ) {
                  routeColor = "#59a200";
                } else if (
                  claimedTeamId === "0076f246-bf3c-4900-aadd-87b9a9a37452"
                ) {
                  routeColor = "#ff0000";
                }
              }
              ...
              console.warn(
                "Source or destination city not found for connection:",
                connection,
              );
            }
          });
          ...
          if (jwtToken) {
            const userLocationsResponse = await fetch(
              `${base_url}/user-locations/`,
              {
                method: "GET",
                headers: {
                  Authorization: `Bearer ${jwtToken}`,
            ...
            const activeLocations = userLocationsData.filter(
