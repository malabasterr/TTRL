      ...
      try {
        await Promise.all([
          fetchClaimedDistances(
            "1446e8a4-350c-4aa1-a997-c05fb87ef102",
            setTeamOneDistances,
          ),
          fetchClaimedDistances(
            "79cd421b-81d4-4b00-8b59-da9e7560dc4b",
            setTeamTwoDistances,
          ),
          fetchClaimedDistances(
            "0076f246-bf3c-4900-aadd-87b9a9a37452",
            setTeamThreeDistances,
          ),
        ]);
      } catch (error) {
      ...
      <div className="backgroundHP">
        ...
        <div className="totalDistancesContainer">
          {teamOneDistances.map((teamDistance) => (
            <div
              key={teamDistance.team_id}
              className="teamOneDistanceContainer"
            >
              <h2 className="teamName">{teamDistance.team_name}</h2>
              <Link to="/WMSummary">
                <h2 className="teamDistance">
                  {teamDistance.claimed_distance} km
                </h2>
              </Link>
            </div>
          ))}
          {teamTwoDistances.map((teamDistance) => (
            <div
              key={teamDistance.team_id}
              className="teamTwoDistanceContainer"
            >
              <h2 className="teamName">{teamDistance.team_name}</h2>
              <Link to="/JHSummary">
                <h2 className="teamDistance">
          ...
          {teamThreeDistances.map((teamDistance) => (
            <div
              key={teamDistance.team_id}
              className="teamThreeDistanceContainer"
            >
              <h2 className="teamName">{teamDistance.team_name}</h2>
              <Link to="/MWSummary">
                <h2 className="teamDistance">
        ...
        <div className="topLineButtons">
          ...
          <div className="unclaimRouteButtonContainer">
            <Link to="/unclaimroute">
              <button className="unclaimRouteButton">UNCLAIM</button>
            </Link>
          </div>
        </div>
        <div className="bottomLineButtons">
          <div className="claimBonusSiteButtonContainer">
            <Link to="/claimbonussite">
              <button className="claimBonusSiteButton">
                CLAIM A BONUS SITE
              </button>
            </Link>
          </div>
          <div className="shareLocationButtonContainer">
            <Link to="/sharelocation">
              <button className="shareLocationButton">TRACK LOCATION</button>
            </Link>
          </div>
          <div className="drawScrewYouCardButtonContainer">
            <Link to="/drawscrewyoucard">
              <button className="drawScrewYouCardButton">
                DRAW SCREW YOU CARD
              </button>
            </Link>
          </div>
